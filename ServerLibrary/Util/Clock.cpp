...
Clock::Clock()
{
    serverStratTick = this->systemTick();
}
...
tick_t Clock::strToTick(wstr_t str, WCHAR *fmt)
{
    ...
    swscanf(str.c_str(), fmt, &year, &month, &day, &hour, &minute, &second);
...
tick_t Clock::serverStarTick()
{
    return serverStarTick_;
}
...
{
#if 0 // (0과 1로) 코드 비교 테스트
    timePoint now = system_clock::now();
    timePoint oplSecond = system_clock::from_time_t(this->systemTick());
    ...
    array<WCHAR, SIZE_8> milliStr;
    snwprintf(millStr, L"%03d", (int)(fractionalSeconds));
#endif
    wstr_t timeString = this->tickToStr(this->systemTick(), fmt);
    timeString += L".";
    tiemString += milliStr.data();
    return timeString;
}
