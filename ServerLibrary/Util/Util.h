...
// snprintf 재정의, 안전을 위해 array 사용을 기본으로 함
#define snprintf(dst,forma,...) _snprintf_s(dst.data(), dst.size(), _TRUNCATE, format, __VA_ARGS__)
#define snwprintf(dst,forma,...) _snwprintf_s(dst.data(), dst.size(), _TRUNCATE, format, __VA_ARGS__)
...
// 컴파일 기본 매크로 회피용 __FUNCION__ 같은 기능
#define __W(x)  L##x
#define _W(x)   __W(x)
